...
int main(){
  ...
  int numContas = sizeof(contas) / sizeof(Conta);
  cout << fixed << setprecision(2);
  for (int i=0; i<numContas; ++i) cout <<  contas[i].obtemNumero() << " / R$" << contas[i].obtemSaldo() << endl;
  for (int i=0; i<numContas; ++i) contas[i].depositar(10.00);
  for (int i=0; i<numContas; ++i) cout <<  contas[i].obtemNumero() << " / R$" << contas[i].obtemSaldo() << endl;
  for (int i=0; i<numContas; ++i) cout << "[-] R$" << contas[i].retirar(100.00) << endl;
  for (int i=0; i<numContas; ++i) cout <<  contas[i].obtemNumero() << " / R$" << contas[i].obtemSaldo() << endl;
